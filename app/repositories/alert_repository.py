...
class AlertRepository:
    ...
    def fetch_and_claim_alert_by_id(self, alert_id: str) -> Optional[Dict[str, Any]]:
        """
        Atomically fetches a specific alert by alert_id from Neon DB.
        If its status is OPEN, updates it to UNDER_INVESTIGATION.
        Loads all related entity data for investigation context.
        """
        try:
            alert = (
                self.db.query(Alert)
    ...
    def fetch_alert_read_only(self, alert_id: str) -> Optional[Dict[str, Any]]:
        """
        Loads an alert and related entities without claiming or modifying status (READ-ONLY).
        """
        try:
    ...
    def complete_alert(self, alert_id: str, new_status: str = "RESOLVED") -> Optional[Alert]:
        """
        Transitions an alert currently UNDER_INVESTIGATION to RESOLVED (or ESCALATED) in Neon DB
